...
def plot_seasonal_comparison(
    ...
    """Compares the RMSE performance of two different experiment runs (e.g., with vs without seasonality).

    Args:
        first_path: Path to the first JSON results file.
        second_path: Path to the second JSON results file.
        first_label: Legend label for the first dataset.
        second_label: Legend label for the second dataset.
    """
    # Load normal results
    ...
    filtered_normal = [
        exp for exp in data_normal["experiments"]
        if exp["context_length"] == 256
    ]

    session_normal = {
        f"horizon_{exp['horizon']}": exp["metrics"]
        for exp in filtered_normal
    }

    horizon_keys = list(session_normal.keys())
    horizons = [int(x.split('_')[-1]) for x in horizon_keys]
    models = list(session_normal[horizon_keys[0]].keys())

    rmse_normal = {
        model: [session_normal[h_key][model]['RMSE'] for h_key in horizon_keys]
        for model in models
    }

    # Load seasonal results
    with open(second_path, "r") as f:
        data_seasonal = json.load(f)

    filtered_seasonal = [
        exp for exp in data_seasonal["experiments"]
        if exp["context_length"] == 256
    ]
    ...
    rmse_seasonal = {
        model: [session_seasonal[h_key][model]['RMSE'] for h_key in horizon_keys]
        for model in models
    }
